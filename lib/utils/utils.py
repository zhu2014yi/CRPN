...
def load_dataset(dataset):
    ...
    if 'OTB' in dataset:
        ...
        base_path=join('/ssd',dataset)
        json_path=join('/ssd',dataset+'.json')
        ...
        base_path=join('/ssd',dataset)
        list_path=join('/ssd',dataset+'/list.txt')
...
def IoU(rect1, rect2):
    ...
    :param rect1: list 4  each consist of 5*17*17 numpy
    :param rect2:  tuple 4
    :return:
    """
...
def G_IoU(rect1,rect2):
    """

    :param rect1:  list 4 each consist of 5*17*17 numpy  x1,y1,x2,y2
    :param rect2:  tuple x1,y1,x2,y2
    :return:
    """
    x1, y1, x2, y2 = rect1[0], rect1[1], rect1[2], rect1[3]
    tx1, ty1, tx2, ty2 = rect2[0], rect2[1], rect2[2], rect2[3]
    ...
    closure_xx1=np.minimum(tx1, x1)
    closure_yy1=np.minimum(ty1, y1)
    closure_xx2 = np.maximum(tx2, x2)
    closure_yy2 = np.maximum(ty2, y2)

    closure_ww = np.maximum(0, closure_xx2 - closure_xx1)
    closure_hh = np.maximum(0, closure_yy2 - closure_yy1)
    closure=closure_ww*closure_hh
    area1 = (x2 - x1) * (y2 - y1)
