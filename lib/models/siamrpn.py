...
from  loss import  *
from lib.utils.bbox_utils import *
class SiamRPN_(nn.Module):
    ...
    def forward(self, epoch,template, search, label_cls=None, label_reg=None, reg_weight=None, sum_weight=None):
    ...
    def giou_loss(self,pred, target, weights,eps=1e-6):
        ...
        Return:
            Tensor: Loss tensor.
        """
        """select bbox_overlaps_ciou,bbox_overlaps_diou,bbox_overlaps_giou,bbox_overlaps_iou,bbox_overlaps_uiou"""
        dious = bbox_overlaps_diou(pred, target)
        #gious_test=gious.detach().cpu().numpy()
        loss = (1.0 - dious).float()
        #loss_test = loss.reshape(16*17,5*17).detach().cpu().numpy()
        b, _, _ ,_ = weights.size()
