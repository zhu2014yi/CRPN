...
parser = argparse.ArgumentParser(description='parameters for SiamRPN tracker')
parser.add_argument('--arch', dest='arch', default='SiamRPNRes22',
                    help='architecture of pretrained model')
parser.add_argument('--resume', default='/ssd/zhuyi/Code/snapshot_orig/checkpoint_e28.pth', type=str,
                    help='resumed checkpoint')
parser.add_argument('--gpu_nums', default=2, type=int, help='gpu numbers')
parser.add_argument('--dataset', default='VOT2016', type=str, metavar='DATASET', help='dataset')
...
def fitness(indv):
    ...
    if args.dataset.startswith('OTB'):
        ...
        config['hp'] = dict()
        config['hp']['scale_step'] = scale_step
        config['hp']['penalty_k'] = scale_penalty
        config['hp']['lr'] = scale_lr
        config['hp']['window_influence'] = window_influence
        config['hp']['big_sz']=255
        config["hp"]['small_sz']=255

        eao = eao_vot_rpn(tracker, net, config)
